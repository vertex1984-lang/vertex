...
- 首页 Featured Products 区：桌面端 bento 网格（容器 max-w-[1800px]、左右 5:12——宽度按「右侧 3 列小卡保持原 2×2 时代卡宽」推算，左侧两张 1:1 焦点大卡上下排列 = 位次 1/2 + 右侧 3×2 小卡 = 位次 3–8，上排 3/4/5、下排 6/7/8）；移动端为横向滚动条（8 张统一大小 V2ProductCard），两套布局 `lg:hidden` / `hidden lg:block` 互斥。**选品为类目配额制**（2026-09 用户定，`featured-sections.ts` 的 `getFeaturedProducts()`，共 8 个卡位）：类目权重 = 该类目在售产品数占总在售产品数（不含 Others）的比例，配额 = 比例 × 8 四舍五入再按误差修正到恰好 8；类目内部按 `sortByWeight()` 总分降序；卡片顺序按类目份额降序整组排列（份额高的类目靠前）。原 FEATURED_ASINS 人工清单已废止；首页 Best Sellers 区块排除 Featured 已展示产品避免重复。
