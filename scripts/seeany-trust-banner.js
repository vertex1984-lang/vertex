/**
 * seeany-trust-banner.js — 生成首页信任横幅配图（2026-09-22，替代 Review 轮播的新模块）
 * 用法: node scripts/seeany-trust-banner.js
 * 21:9 横板大图，左侧大面积柔和留白（供叠压白字文案），输出 public/images/brand/trust-banner.png
 * （生成后用 PIL 转 webp：python -c "from PIL import Image; Image.open('trust-banner.png').save('trust-banner.webp', quality=82)"）
 */
const fs = require('fs');
...
const JOB = {
  name: 'trust-banner',
  ratio: '21:9',
  prompt: `超宽幅家居品牌横幅：明亮温暖的开放式居家空间，右侧主体为陈列整齐的米色亚麻床品、抱枕与羊毛毯，木质家具与绿植点缀，左侧留出大面积干净的米白色墙面作为负空间（便于后期叠加文字），构图横向开阔，光线柔和。${STYLE}`,
};
